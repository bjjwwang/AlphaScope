...
class ADARNN(Model):
    ...
    def fit(
        ...
        train_loader_list = [get_stock_loader(df, self.batch_size, d_feat=self.d_feat, len_seq=self.len_seq) for df in train_splits]
        ...
        for step in range(self.n_epochs):
            ...
            if step == 0 or valid_score > best_score:
...
class data_loader(Dataset):
    def __init__(self, df, d_feat=6, len_seq=60):
...
def get_stock_loader(df, batch_size, d_feat=6, len_seq=60, shuffle=True):
    train_loader = DataLoader(data_loader(df, d_feat=d_feat, len_seq=len_seq), batch_size=batch_size, shuffle=shuffle)
    return train_loader
